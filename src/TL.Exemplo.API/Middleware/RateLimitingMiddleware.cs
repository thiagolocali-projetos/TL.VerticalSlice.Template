...
public class RateLimitingMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        var history = RequestHistories.AddOrUpdate(
            ...
            (key, existing) =>
            {
                // Limpar requisições antigas (anterior a 1 minuto)
        ...
        // Usar lock apenas para operações síncronas
        lock (history)
        {
        ...
        if (isRateLimited)
        {
            _logger.LogWarning("⚠️ Rate limit exceeded para IP: {ClientIp}", clientIp);
            ...
            var response = new
            {
                ...
                message = $"Você excedeu o limite de {_requestsPerMinute} requisições por minuto",
                retryAfter = 60
            };
    ...
    {
        // Tentar obter do header X-Forwarded-For (quando atrás de proxy)
    ...
    /// <summary>
    /// Classe auxiliar para rastrear histórico de requisições.
    /// </summary>
...
/// <summary>
/// Extensões para registrar o middleware de rate limiting.
/// </summary>
