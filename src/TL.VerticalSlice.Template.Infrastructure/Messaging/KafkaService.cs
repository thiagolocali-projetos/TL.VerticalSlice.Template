...
public class KafkaService : IKafkaService, IDisposable
{
    ...
    public KafkaService(ILogger<KafkaService> logger)
    {
        ...
        _logger.LogInformation("ðŸ“¡ Kafka conectado em localhost:9092");
    }
    ...
    public async Task ProduzirAsync<T>(string topico, string key, T mensagem, CancellationToken ct = default)
    {
        ...
        _logger.LogInformation(
            "ðŸ“¤ Kafka produzido: TÃ³pico={Topico}, Key={Key}, Partition={Partition}, Offset={Offset}",
    ...
    public Task ConsumirAsync<T>(string topico, Func<T, Task> callback, CancellationToken ct = default)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                while (!ct.IsCancellationRequested)
                {
                    ...
                    {
                        _logger.LogInformation(
                            "ðŸ“¥ Kafka consumido: TÃ³pico={Topico}, Key={Key}, Offset={Offset}",
        ...
        _logger.LogInformation("ðŸ‘‚ Kafka escutando tÃ³pico: {Topico}", topico);
    ...
    {
        _producer?.Dispose();
        _logger.LogInformation("ðŸ“¡ Kafka desconectado");
    }
}
